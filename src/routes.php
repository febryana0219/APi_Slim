...
return function (App $app) {
    ...
    $app->group('/api', function(\Slim\App $app) {
        ...
        //pencarian data produk berdasarkan KodeProduk atau NamaProduk
        $app->get("/produk/search/", function (Request $request, Response $response, array $args){
            $keyword = $request->getQueryParam("keyword");
            $sql = "SELECT * FROM produk WHERE KodeProduk LIKE '%$keyword%' OR NamaProduk LIKE '%$keyword%'";
            $stmt = $this->db->prepare($sql);
            $stmt->execute();
            $mainCount=$stmt->rowCount();
            $result = $stmt->fetchAll();
